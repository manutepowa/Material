<?php  
	...
	if ($q==0) {
		...
		$query = "SELECT prestamo.id_prestamo, material.descripcion, persona.nombre, lugar.lugar, usuario.id_usuario, usuario.nombre_usuario, prestamo.fecha_prestamo, prestamo.fecha_devolucion, prestamo.observaciones FROM prestamo, material, persona, lugar, usuario, lineas_prestamo WHERE prestamo.id_persona = persona.id_persona AND prestamo.id_lugar = lugar.id_lugar AND lineas_prestamo.id_material = material.id_material AND usuario.id_usuario = prestamo.id_usuario AND lineas_prestamo.id_prestamo = prestamo.id_prestamo AND prestamo.eliminado = 0 ORDER BY prestamo.fecha_prestamo DESC, prestamo.id_prestamo;";
	}
	else{
